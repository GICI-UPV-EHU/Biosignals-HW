...
SemaphoreHandle_t SemCore1;
SemaphoreHandle_t SemCore0;
...
void setup() 
{
  SemCore0 = xSemaphoreCreateBinary();
  SemCore1 = xSemaphoreCreateBinary();
  ...
  xTaskCreate(TareaGSR_code, "Task GSR", 4960, NULL, 15, NULL);   

  xTaskCreate(TareaPOX_code, "Task POX", 4960, NULL, 20, NULL); 

  xTaskCreate(TareaECG_code, "Task POX", 4960, NULL, 30, NULL);   
}
...
void TareaGSR_code (void* paremeter)
{
  ...
  char buffer[50];
  TickType_t t_ult;
  const TickType_t freq = 1000; // Se establace que haya 1 muestra cada 10 ticks --> 1000 muestras/s
