...
def animate(i, x_Tiempo, y_Ir, y_Rojo):
   ...
   b,a= signal.butter(3, w, 'lowpass' )
   
   y_Ir_filtro = signal.filtfilt(b, a, y_Ir, padlen=0)
   y_Rojo_filtro = signal.filtfilt(b, a, y_Rojo, padlen=0)
   ...
   graf_ir.plot(x_Tiempo, y_Ir_filtro, linewidth = 2, color = 'r')
   graf_rojo.plot(x_Tiempo, y_Rojo_filtro, linewidth =2, color = 'b')
   
# función que llamará periodicamente a 'animate'

ani = animation.FuncAnimation(fig, animate, fargs=(x_Tiempo, y_Ir, y_Rojo), interval =100)
plt.show()
